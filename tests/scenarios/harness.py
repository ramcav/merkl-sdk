"""One rig the five scenarios share, wired the way a real deployment is.
...
real sealed state file in a temp directory; the policy is a real signed document;
the approvals are real signatures. Only the *rail* is swappable, which is the
point of the parametrisation: the same scenario must reach the same verdict
...
INVOICE_HASH = SHA256Hash.from_bytes(b"invoice INV-2026-0042.pdf").hex()
ORIGIN = "https://app.merkl.ai"
RP_ID = "app.merkl.ai"
...
ADMIN = Ed25519Party("scenario-admin")
AGENT = Ed25519Party("scenario-agent")
ALICE = Ed25519Party("scenario-alice")
CAROL = Ed25519Party("scenario-carol")
BOB = PasskeyParty(fixtures.WEBAUTHN_SCALAR)
...
def build_rig(
    ...
    *,
    ...
    state = SealedStateStore(tmp_path / "state", document.treasury, keystore.seal_key())
    engine = SignerEngine(
        policy=sign_policy(document),
        keystore=keystore,
        state=state,
        clock=clock,
